...
class LicenseController extends Controller
{
	...
	public function get_latest_version(){
		...
		//);
		$get_data = '{"status":true,"message":"Latest version of Openai Davinci is v1.3","product_name":"Openai Davinci","latest_version":"v1.3","release_date":"2023-03-06","summary":"Update For Security Issue"}';
	...
	public function activate_license($license, $client, $create_lic = true){
        if($license != 'Template (Www.Mobanzhu. Com) cracking completed'){
            $response = json_decode('{"status":false,"message":"Not Activated! license must."}');
        } else {
            $data_array =  array(
                "product_id"  => $this->product_id,
            ...
            //	json_encode($data_array)
            //);
            $get_data = base64_decode('');
            $response = json_decode($get_data, true);
        }
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			if(strtotime($today) >= strtotime($_SESSION["572c216d78723bd"])){
				...
				//	json_encode($data_array)
				//);
				$get_data = '{"status":true,"message":"Verified! Thanks for purchasing Openai Davinci.","data":null}';
